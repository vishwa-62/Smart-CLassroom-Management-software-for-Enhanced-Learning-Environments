...
const ChildMarks = () => {
  ...
  const fetchMarks = async () => {
    try {
      const res = await api.get('/marks/report-card/1');
      if (res.data.success) {
  ...
  if (loading) return <LoadingSpinner label="Fetching Child Exam Scores..." />;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">
            Academic Scorecard: Alex Johnson
          </h1>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
            Subject-wise examination marks and evaluation standing.
          </p>
        </div>
        <button
        ...
        >
          <Printer className="w-4 h-4" /> Download Official PDF Card
        </button>
      </div>

      <div className="p-6 bg-gradient-to-r from-purple-900 to-slate-900 text-white rounded-3xl shadow-xl flex items-center justify-between">
        <div>
          <span className="text-xs font-bold text-purple-300 uppercase tracking-widest">Midterm Grade Summary</span>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {marks.map((m, idx) => (
                <tr key={idx} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition">
                  ...
                  <td className="p-4 text-slate-500">{m.remarks || 'Excellent'}</td>
                </tr>
              ))}
            </tbody>
          </table>
